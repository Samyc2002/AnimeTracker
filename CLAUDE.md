...
- **`lib/admin.ts`** -- Shared admin check module: `getEmailSets()` resolves `ADMIN_EMAILS`/`TEST_ACCOUNTS` to user ID sets, `isAdmin()` for server components, `getCallerUserIdFromRequest()` for API routes
- **`components/ui/`** -- Extracted reusable UI components: `Spinner`, `StatusBadge`, `DashboardInput`, `AuthInput`, `AuthSubmitButton`
- **`/dev/inventory`** -- Admin-gated visual inventory page rendering every distinct UI element; preparation for cosmetic pack engine
...
- `ADMIN_EMAILS` -- comma-separated emails for admin-gated routes (e.g., `/dev/inventory`, `/api/stats`)
...
118 Vitest tests, ~91% line coverage. Pre-commit hook via Husky runs tests before every commit -- never use `--no-verify`.
